...
module Mutations
  class CreateCustomerProfile < BaseMutation
    description "Create a customer profile for the current user"
    ...
    def resolve(date_of_birth: nil, preferred_language: nil)
      ...
      if user.customer_profile.present?
        return {
          customer_profile: nil,
          errors: [ "Customer profile already exists" ]
        }
      end

      profile = user.build_customer_profile(
